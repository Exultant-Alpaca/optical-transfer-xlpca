...
describe("passphrase helpers", () => {
  it("creates the expected number of readable random parts", () => {
    ...
    expect(PASSPHRASE_PART_COUNT).toBe(12);
...
describe("passphrase cryptography", () => {
  it("derives the same AES-GCM key on both sides and rejects a different phrase", async () => {
    ...
    salt.fill(7);
    const phrase = "amber-river bright-cloud calm-forest copper-moon gentle-pine lunar-stream silver-valley swift-wind warm-meadow quiet-lake golden-hill cedar-comet";
    ...
    const wrongKey = await derivePassphraseKey("amber-river bright-cloud calm-forest copper-moon gentle-pine lunar-stream silver-valley swift-wind warm-meadow quiet-lake golden-hill cedar-moon", salt);
